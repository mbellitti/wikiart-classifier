"""Downloads the urls of the image-pages on wikiart.org. Two pickled
lists will be saved; one 'artworks_urls.pkl' contains the urlsn of the artwork
pages, the other 'skipped_urls.pkl' contains pages from which the urls could
not be read."""
...
def main():
    ...
    artworks_urls, skipped_urls = get_imagepage_urls(test = True)
    path = join('..', 'artwork_urls')
    # save urls
    filename_aw = 'artworks_urls.pkl'
    f_aw = open(join(path, filename_aw), 'wb')
    pickle.dump(artworks_urls, f_aw)
    f_aw.close()
    # save skipped urls
    filename_sk = 'skipped_urls.pkl'
    f_sk = open(join(path, filename_sk), 'wb')
    pickle.dump(skipped_urls, f_sk)
    f_sk.close()
